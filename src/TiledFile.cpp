...
std::vector<TiledFile::Layer> TiledFile::parseLayers(const Json::Value & node)const
{
...
std::vector<TiledFile::TileSet> TiledFile::parseTileSets(const Json::Value & node)const
{
    std::vector<TiledFile::TileSet> tileSets;
	assert(node.isArray());
    return tileSets;
}
std::map<std::string, std::string> TiledFile::parseProperties(const Json::Value & node)const
{
